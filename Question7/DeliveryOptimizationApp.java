...
public class DeliveryOptimizationApp {
    ...
    private RoutePanel routePanel; // Custom panel for visualizing the optimized route
    ...
    private void createAndShowGUI() {
        ...
        deliveryListArea.setBorder(BorderFactory.createTitledBorder("Delivery List (Address, Priority)")); // Add a border with title
        ...
        routePanel = new RoutePanel(); // Create a custom panel for route visualization
    ...
    private class OptimizeButtonListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            try {
                ...
                String deliveryListText = deliveryListArea.getText().trim(); // Get the delivery list text
                if (deliveryListText.isEmpty()) {
                    JOptionPane.showMessageDialog(frame, "Delivery list cannot be empty.", "Error", JOptionPane.ERROR_MESSAGE);
                    return;
                }

                int vehicleCapacity = Integer.parseInt(vehicleCapacityField.getText().trim()); // Parse vehicle capacity
                int distanceConstraint = Integer.parseInt(distanceConstraintField.getText().trim()); // Parse distance constraint
                ...
                new Thread(() -> {
                    ...
                    SwingUtilities.invokeLater(() -> routePanel.setRoute(optimizedRoute));
                }).start();

            } catch (NumberFormatException ex) {
                JOptionPane.showMessageDialog(frame, "Please enter valid numbers for vehicle capacity and distance constraint.", "Input Error", JOptionPane.ERROR_MESSAGE);
            } catch (Exception ex) {
    ...
    private List<DeliveryPoint> parseDeliveryList(String text) {
        ...
        for (String line : lines) {
            ...
            if (parts.length != 2) {
                JOptionPane.showMessageDialog(frame, "Invalid input format. Each line should be in the format: Address, Priority", "Input Error", JOptionPane.ERROR_MESSAGE);
                continue;
            }
            ...
            try {
                ...
                JOptionPane.showMessageDialog(frame, "Invalid priority value. Please enter a valid integer.", "Input Error", JOptionPane.ERROR_MESSAGE);
            }
        }
    ...
    // Custom JPanel class for visualizing the route
    private class RoutePanel extends JPanel {
        private List<Route> routes; // List of routes to be visualized

        public RoutePanel() {
            routes = new ArrayList<>(); // Initialize the list of routes
        }

        public void setRoute(List<Route> routes) {
            this.routes = routes; // Set the routes
            repaint(); // Trigger repaint to visualize the routes
        }
        ...
        protected void paintComponent(Graphics g) {
            ...
            // Example drawing logic (replace with actual visualization)
            g.setColor(Color.BLUE); // Set the color to blue
            g.drawLine(10, 10, 100, 100); // Draw a line as an example

            // Additional drawing code for visualizing routes can be added here
        }
    }
}
